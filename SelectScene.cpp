...
void SelectScene::initialize()
{
	...
	//曲名
	m_text_title.setFont(m_font_list);
	m_text_title.setString("Not Found Title");
	m_text_title.setCharacterSize(75);
	m_text_title.setPosition(1050, 0);
	//アーティスト
	m_text_artist.setFont(m_font_list);
	m_text_artist.setString("Not Found Artist");
	m_text_artist.setCharacterSize(60);
	m_text_artist.setPosition(1400, 100);
	//レベル
	m_text_level.setFont(m_font_list);
	m_text_level.setString("Lv.");
	m_text_level.setCharacterSize(85);
	m_text_level.setPosition(1050, 180);
	//サムネイル
	m_tex_jacket.loadFromFile("data/image/select/no_jacket.png");//本体
	m_jacket.setTexture(m_tex_jacket);
	m_jacket.setPosition(1000, 500 -m_jacket.getGlobalBounds().height/2);
	m_jacket_cover.setTexture(tex.get("select_jacket_cover"));//カバー
	m_jacket_cover.setPosition(m_jacket.getPosition());
...
Scene* SelectScene::update()
{
	...
	//曲名
	m_text_title.setString(cv.from_bytes(musicManager.getItem(config.getNowMusic(config.Num)).music_name));
	//アーティスト
	m_text_artist.setString(cv.from_bytes(musicManager.getItem(config.getNowMusic(config.Num)).artist));
	//レベル
	m_text_level.setString("Lv."+to_string(musicManager.getItem(config.getNowMusic(config.Num)).level[config.getNowMusic(config.Diff)]));
	//サムネイル
...
void SelectScene::render()
{
	...
	windowManager.getWindow()->draw(m_text_title);//曲名
	windowManager.getWindow()->draw(m_text_artist);//アーティスト
	windowManager.getWindow()->draw(m_text_level);//レベル
	windowManager.getWindow()->draw(m_jacket,m_state_jacket);//サムネイル
	windowManager.getWindow()->draw(m_jacket_cover);//サムネイル　カバー
